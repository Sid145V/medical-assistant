...
const DoctorCard: React.FC<{ doctor: Doctor; onBook: (doctor: Doctor) => void }> = ({ doctor, onBook }) => (
  <motion.div 
    className="glass-card p-6 flex flex-col items-center text-center transition-all transform hover:-translate-y-2"
  ...
  >
    ...
    <p className="text-sm text-text-muted mt-1">{doctor.location}</p>
    <motion.button
    ...
    <AnimatePresence>
        {doctor && (
            ...
            >
                ...
                >
                    <h2 className="text-2xl font-bold mb-4 text-text">Book with {doctor.name}</h2>
...
const AppointmentsPage: React.FC = () => {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-center mb-8 text-text">Available Doctors</h1>
