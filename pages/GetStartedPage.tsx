...
const RoleCard: React.FC<{ title: string; description: string; iconPath: string; onClick: () => void }> = ({ title, description, iconPath, onClick }) => (
  <motion.div 
    className="glass-card p-8 transition-all transform hover:-translate-y-2 cursor-pointer text-left shadow-lg hover:shadow-xl"
    onClick={onClick}
    whileHover={{ y: -8, scale: 1.02, boxShadow: '0 20px 40px rgba(6, 20, 30, 0.1)' }}
    transition={{ type: 'spring', stiffness: 300 }}
  >
    <Icon path={iconPath} />
    <h3 className="text-2xl font-bold text-text-light dark:text-text-dark mt-4">{title}</h3>
    <p className="mt-2 text-text-muted-light dark:text-text-muted-dark">{description}</p>
  </motion.div>
);
...
const GetStartedPage: React.FC = () => {
  ...
  const roles = [
      ...
      { title: "Doctor", description: "Manage your appointments and connect with patients.", role: UserRole.DOCTOR, icon: "M12 6.253v11.494m-5.747-8.994l11.494 5.747m-11.494 0l11.494-5.747" },
  ...
  return (
    <div className="text-center max-w-4xl mx-auto">
      <h1 className="text-4xl font-extrabold text-text-light dark:text-text-dark">Join Our Healthcare Community</h1>
