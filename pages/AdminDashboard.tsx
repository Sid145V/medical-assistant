...
const StatCard: React.FC<{ title: string; value: number; icon: string; color: string }> = ({ title, value, icon, color }) => (
    <div className={`glass-card p-6 flex items-center space-x-4 border-l-4 ${color}`}>
      ...
      case 'home':
        return (
          <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-6">
            <StatCard title="Total Patients" value={data.patients.length} icon="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M15 21v-1a6 6 0 00-1.78-4.125a4 4 0 00-6.44 0A6 6 0 003 20v1h12z" color="border-blue-500" />
            <StatCard title="Verified Doctors" value={data.doctors.length} icon="M12 6.253v11.494m-5.747-8.994l11.494 5.747m-11.494 0l11.494-5.747" color="border-green-500" />
            <StatCard title="Registered Shops" value={data.shops.length} icon="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" color="border-yellow-500" />
            <StatCard title="Inbox Messages" value={data.messages.length} icon="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" color="border-purple-500" />
          </div>
        );
          ...
          <div className="overflow-x-auto glass-card">
            <table className="min-w-full">
              <thead className="border-b border-white/20">
                <tr>
                  {headers.map(h => <th key={h} className="py-3 px-4 text-left text-xs font-semibold text-text-muted uppercase tracking-wider">{h}</th>)}
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-primary/5 transition-colors group">
                      {rowData.map((cellData, idx) => (
                        <td key={idx} className="py-3 px-4 text-sm text-text whitespace-nowrap relative">
                           {idx === 0 && (
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8">
        <aside className="md:w-64 flex-shrink-0">
            <h1 className="text-2xl font-bold mb-6 text-text">Admin Panel</h1>
            <nav className="flex md:flex-col space-x-2 md:space-x-0 md:space-y-2 overflow-x-auto">
                {TABS.map(tab => (
                    <button
                    key={tab.id}
                    onClick={() => setActiveTab(tab.id)}
                    className={`flex items-center space-x-3 text-left py-2 px-4 rounded-lg font-semibold transition-colors w-full ${activeTab === tab.id ? 'bg-primary text-white shadow-lg' : 'text-text-muted hover:bg-primary/10 hover:text-text'}`}
                    >
